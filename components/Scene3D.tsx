...
const PRESETS_DARK: Preset[] = [
  // 0: hero — большая сфера справа, мягкая
  {
    position: [2.0, 0.4, 0],
    scale: 0.7,
    rotation: [0.04, 0.18, 0],
    intensity: 1.0, extra: 0.0,
    twist: 0.0, spikes: 0.0,
    squash: [1, 1, 1],
    ringMode: 0.0,
    explosionT: 0.0, chaos: 0.0,
    opacity: 0.9, particleOpacity: 0.55, dynOpacity: 0.0,
  ...
  // 1: myself — рамка для фото. Сфера плоская (squash Z~0), кольцо видно
  {
    ...
    intensity: 1.35, extra: 0.18,
    twist: 0.0, spikes: 0.0,
    squash: [1.25, 1.55, 0.07],   // плоский овал по XY
    ringMode: 0.92,                // сильный эффект рамки
    explosionT: 0.0, chaos: 0.0,
    opacity: 0.85, particleOpacity: 0.3, dynOpacity: 0.0,
    colorA: '#0a1530', colorB: '#4fa3ff', colorC: '#c084fc',
  },
  ...
  // 3: featured — частицы хаотично летят
  {
    ...
    colorA: '#0a2030', colorB: '#22d3ee', colorC: '#ec4899',
  },
  ...
  // 4: projects — максимальный хаос
  {
    ...
    colorA: '#1a0a30', colorB: '#7c3aed', colorC: '#f472b6',
  },
  ...
  // 6: finalCta — тихая сфера в центре, текст читается поверх
  {
    ...
    ringMode: 0.0,
    explosionT: 0.0, chaos: 0.0,
    opacity: 0.22,               // намеренно тусклая
  ...
  // 7: footer — маленький свёрнутый шар
  {
    ...
    colorA: '#0a1530', colorB: '#3a5ec0', colorC: '#5a4ad9',
  },
];
...
const PRESETS_LIGHT: Preset[] = [
  // 0: hero
  {
    position: [2.0, 0.4, 0], scale: 0.7,
    rotation: [0.04, 0.18, 0],
    intensity: 1.0, extra: 0.0, twist: 0, spikes: 0,
    squash: [1, 1, 1], ringMode: 0.0,
    explosionT: 0.0, chaos: 0.0,
    opacity: 0.7, particleOpacity: 0.4, dynOpacity: 0.0,
    colorA: '#1a2540', colorB: '#2563eb', colorC: '#7c3aed',
  },
  // 1: myself — рамка для фото (светлая тема)
  {
    position: [2.3, 0.05, 0], scale: 0.72,
    rotation: [0.015, 0.06, 0.01],
    intensity: 1.2, extra: 0.15, twist: 0, spikes: 0,
    squash: [1.25, 1.55, 0.07], ringMode: 0.92,
    explosionT: 0.0, chaos: 0.0,
    opacity: 0.7, particleOpacity: 0.22, dynOpacity: 0.0,
    colorA: '#1a2540', colorB: '#2563eb', colorC: '#7c3aed',
  },
  // 2: logos
  {
    ...
    colorA: '#1a2540', colorB: '#2563eb', colorC: '#6d28d9',
  },
  // 3: featured
  {
    ...
    colorA: '#0e2a34', colorB: '#0891b2', colorC: '#db2777',
  },
  // 4: projects
  {
    ...
    colorA: '#231246', colorB: '#7c3aed', colorC: '#db2777',
  },
  // 5: experience / tech-stack
  {
    ...
    colorA: '#1a2540', colorB: '#3b58db', colorC: '#4f6cff',
  },
  // 6: finalCta — очень тусклая (светлая тема)
  {
    ...
    squash: [1, 1, 1], ringMode: 0,
    explosionT: 0.0, chaos: 0.0,
    opacity: 0.13,               // ещё тусклее для светлой темы
  ...
  // 7: footer
  {
    ...
    colorA: '#1a2540', colorB: '#3a5ec0', colorC: '#5a4ad9',
  },
];
...
interface SharedRefs {
  ...
  /** true когда мы на /projects/* — сфера должна исчезнуть */
...
function MorphSphere({ mouseRef, themeRef, stageRef, sectionFloatRef, spherePosRef, dissolveRef }: SharedRefs) {
  ...
  useFrame((state, delta) => {
    ...
    // На странице проектов — сфера мгновенно прозрачна
...
function BackgroundParticles({ themeRef, stageRef, sectionFloatRef, dissolveRef }: Omit<SharedRefs,'mouseRef'|'spherePosRef'>) {
  ...
  useFrame((_, delta) => {
    ...
    // На странице проектов — мгновенно прозрачны
    if (dissolveRef.current) {
      mat.opacity = 0;
      return;
    }
...
function DynamicParticles({ themeRef, stageRef, sectionFloatRef, spherePosRef, dissolveRef }: Omit<SharedRefs,'mouseRef'>) {
  ...
  useFrame((state, delta) => {
    ...
    if (dissolveRef.current) {
      // Страница проектов — мгновенно прозрачны
      u.uOpacity.value  = 0;
      u.uExplodeT.value = 0;
      return;
...
export default function Scene3D() {
  ...
  // Инициализируем синхронно — чтобы на /projects/* сфера не мелькала при первом рендере
  ...
  // Dissolve при входе на /projects/* — без usePathname, просто window.location
  useEffect(() => {
    const check = () => {
      dissolveRef.current = window.location.pathname.startsWith('/projects');
    };
    check();
    // popstate = браузер назад/вперёд
    window.addEventListener('popstate', check);
    // Next.js App Router меняет URL без popstate → слушаем через MutationObserver на title
    // (title меняется при каждой навигации)
